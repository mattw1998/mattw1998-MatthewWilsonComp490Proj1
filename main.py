...
def insert_multi_json_to_sql(json_obj_list):
    insert_statement = ('INSERT INTO jobs (id, title, company, description, image_url, location, employment_type,'
                        ' date_posted, salary_range, job_providers) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    for i in json_obj_list:
        try:
            job_dict = dict(i)
            insert_data = (job_dict.get('id'), job_dict.get('title'), job_dict.get('company'),
                           job_dict.get('description'), job_dict.get('image'), job_dict.get('location'),
                           job_dict.get('employment_type'), job_dict.get('date_posted'), job_dict.get('salary_range'),
...
def insert_single_json_to_sql(json_obj_list):
    insert_statement = ('INSERT INTO jobs (id, site, job_url, job_url_direct, title, company, location,'
    ...
    for i in json_obj_list:
        try:
            job_dict = dict(i)
            insert_data = (job_dict.get('id'), job_dict.get('site'), job_dict.get('job_url'),
                           job_dict.get('job_url_direct'), job_dict.get('title'), job_dict.get('company'),
                           job_dict.get('location'), job_dict.get('job_type'), job_dict.get('date_posted'),
                           job_dict.get('salary_source'), job_dict.get('interval'), job_dict.get('min_amount'),
                           job_dict.get('max_amount'), job_dict.get('currency'), job_dict.get('is_remote'),
                           job_dict.get('job_level'), job_dict.get('job_function'), job_dict.get('company_industry'),
                           job_dict.get('listing_type'), job_dict.get('emails'), job_dict.get('description'),
                           job_dict.get('company_url'), job_dict.get('company_url_direct'),
                           job_dict.get('company_addresses'), job_dict.get('company_num_employees'),
                           job_dict.get('company_revenue'), job_dict.get('company_description'),
                           job_dict.get('logo_photo_url'), job_dict.get('ceo_name'))
            cursor.execute(insert_statement, insert_data)
            job_dict.clear()
        except sqlite3.IntegrityError:
            print('Entry', i, 'already exists in table')
